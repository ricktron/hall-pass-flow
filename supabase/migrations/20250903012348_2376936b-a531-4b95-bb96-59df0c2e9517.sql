-- =================================================================
-- COMPREHENSIVE ANALYTICS REBUILD MIGRATION
-- This migration completely rebuilds the analytics system to fix
-- all broken views and functions with correct column mappings
-- =================================================================

-- Step 1: Drop all existing analytics objects
...
DROP FUNCTION IF EXISTS public.get_analytics_summary(text);
DROP FUNCTION IF EXISTS public.get_analytics_return_rate(text);
DROP FUNCTION IF EXISTS public.get_analytics_by_period(text);
DROP FUNCTION IF EXISTS public.get_analytics_by_destination(text);
DROP FUNCTION IF EXISTS public.get_analytics_frequent_flyers(text);
DROP FUNCTION IF EXISTS public.get_analytics_longest_passes(text);
DROP FUNCTION IF EXISTS public.get_analytics_avg_minutes(text);
DROP FUNCTION IF EXISTS public.get_passes_by_day_of_week(text);
DROP FUNCTION IF EXISTS public.get_weekly_heatmap_data(text);
DROP FUNCTION IF EXISTS public.get_behavioral_insights(text);
DROP FUNCTION IF EXISTS public.get_schedule_type_analysis(text);

-- Step 2: Create the base view with correct column names
...
SELECT 'day'::text AS window, destination, count(*) AS passes, sum(duration) AS minutes_out, percentile_cont(0.5) WITHIN GROUP (ORDER BY duration) AS median_min FROM hp_base WHERE timeout >= date_trunc('day', now()) AND destination IS NOT NULL GROUP BY 1, 2 UNION ALL
SELECT 'week'::text AS window, destination, count(*) AS passes, sum(duration) AS minutes_out, percentile_cont(0.5) WITHIN GROUP (ORDER BY duration) AS median_min FROM hp_base WHERE timeout >= date_trunc('week', now()) AND destination IS NOT NULL GROUP BY 1, 2 UNION ALL
SELECT 'month'::text AS window, destination, count(*) AS passes, sum(duration) AS minutes_out, percentile_cont(0.5) WITHIN GROUP (ORDER BY duration) AS median_min FROM hp_base WHERE timeout >= date_trunc('month', now()) AND destination IS NOT NULL GROUP BY 1, 2 UNION ALL
SELECT 'quarter'::text AS window, destination, count(*) AS passes, sum(duration) AS minutes_out, percentile_cont(0.5) WITHIN GROUP (ORDER BY duration) AS median_min FROM hp_base WHERE timeout >= date_trunc('quarter', now()) AND destination IS NOT NULL GROUP BY 1, 2 UNION ALL
SELECT 'all'::text AS window, destination, count(*) AS passes, sum(duration) AS minutes_out, percentile_cont(0.5) WITHIN GROUP (ORDER BY duration) AS median_min FROM hp_base WHERE destination IS NOT NULL GROUP BY 1, 2;
...
-- Step 4: Create all analytics functions
