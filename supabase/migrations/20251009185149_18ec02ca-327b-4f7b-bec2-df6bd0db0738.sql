-- Fix the disruption score calculation in get_full_analytics function
...
BEGIN
    ...
    WITH period_schedule AS (
        -- Define class periods with start and end times
    ...
    period_zones AS (
        -- Calculate Red and Green zones for each period
        SELECT 
            period,
            period_start,
            period_end,
            ...
            -- Red Zone: Middle 60% (from 20% to 80%)
    ...
    passes_with_scores AS (
        -- Join passes with period zones and calculate individual scores
        SELECT
            bp.student_name,
            bp.duration_min,
