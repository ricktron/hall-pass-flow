-- Add disruption score calculation to get_full_analytics function
...
BEGIN
    ...
    -- Get heatmap data - FIXED: Now handles NULL/empty period values
    WITH heatmap_cte AS (
        SELECT
            CASE EXTRACT(DOW FROM timeout)::INTEGER
                WHEN 1 THEN 'Monday'
        ...
        WHERE timeout >= start_time 
          ...
          AND timeout IS NOT NULL
          AND COALESCE(was_auto_closed, false) = false
        GROUP BY EXTRACT(DOW FROM timeout)::INTEGER, COALESCE(NULLIF(TRIM(period), ''), 'Unknown')
    ...
    -- Define periods with time ranges (simplified school schedule)
    -- Red Zone: middle 60%, Green Zone: first and last 20%
    WITH period_times AS (
        SELECT 'A' AS period, '07:55:00'::time AS start_time, '09:10:00'::time AS end_time
    ...
    pass_with_zones AS (
        SELECT
            ...
            bp.timeout,
            bp.period,
            pt.start_time AS period_start,
            pt.end_time AS period_end,
            EXTRACT(EPOCH FROM (pt.end_time - pt.start_time)) / 60.0 AS period_duration_min,
            EXTRACT(EPOCH FROM (bp.timeout::time - pt.start_time)) / 60.0 AS minutes_into_period,
            CASE
                WHEN EXTRACT(EPOCH FROM (bp.timeout::time - pt.start_time)) / 60.0 <= 
                     (EXTRACT(EPOCH FROM (pt.end_time - pt.start_time)) / 60.0 * 0.2) 
                THEN 'green'
                WHEN EXTRACT(EPOCH FROM (bp.timeout::time - pt.start_time)) / 60.0 >= 
                     (EXTRACT(EPOCH FROM (pt.end_time - pt.start_time)) / 60.0 * 0.8) 
                THEN 'green'
                ELSE 'red'
            END AS zone
        FROM public.bathroom_passes bp
        LEFT JOIN period_times pt ON TRIM(bp.period) = pt.period
    ...
    disruption_calc AS (
        SELECT
            ...
            SUM(
                CASE 
                    WHEN zone = 'red' THEN duration_min * 3
                    WHEN zone = 'green' THEN duration_min * 1
                    ELSE 0
                END
            ) AS disruption_score
        FROM pass_with_zones
        GROUP BY student_name
    ...
    SELECT COALESCE(json_agg(
        json_build_object(
            'student_name', dc.student_name,
            'disruption_score', ROUND(dc.disruption_score::numeric, 1)
        )
    ), '[]'::json)::jsonb INTO disruption_scores_data
    FROM disruption_calc dc;
