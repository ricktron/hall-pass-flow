-- Fix heatmap data query to use same date filtering as day of week chart
...
BEGIN
    ...
    -- Get heatmap data - FIXED: using same date filtering as day_of_week query
    WITH heatmap_cte AS (
        SELECT
            CASE EXTRACT(DOW FROM timeout)::INTEGER
                WHEN 1 THEN 'Monday'
        ...
        HAVING CASE EXTRACT(DOW FROM timeout)::INTEGER
                WHEN 1 THEN 'Monday'
                WHEN 2 THEN 'Tuesday' 
                WHEN 3 THEN 'Wednesday'
                WHEN 4 THEN 'Thursday'
