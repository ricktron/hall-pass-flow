-- =================================================================
-- COMPLETE ANALYTICS ARCHITECTURE REFACTOR
-- This migration replaces all individual analytics functions with a single
-- consolidated function that returns all data in one JSONB object
-- =================================================================
...
-- Step 2: Create the single consolidated analytics function
...
BEGIN
    ...
    -- Get summary data (passes and total minutes)
    WITH summary_cte AS (
        SELECT 
            COUNT(*) AS passes,
            COALESCE(SUM(duration_min), 0) AS total_minutes
        FROM public.bathroom_passes
        WHERE timeout >= start_time AND timeout < end_time
    )
    ...
    WITH heatmap_cte AS (
        ...
        WHERE timeout >= start_time AND timeout < end_time
          AND timeout IS NOT NULL
          AND period IS NOT NULL
          AND EXTRACT(DOW FROM timeout) BETWEEN 1 AND 5
...
GRANT EXECUTE ON FUNCTION public.get_full_analytics(TEXT) TO authenticated;