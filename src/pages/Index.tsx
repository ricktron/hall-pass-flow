...
import AmIStillOut from "@/components/AmIStillOut";

const Index = () => {
  const [currentView, setCurrentView] = useState<'select' | 'student' | 'teacher' | 'status'>('select');
  ...
  if (currentView === 'status') {
    return <AmIStillOut onBack={handleBackToSelection} />;
  }
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <Card className="hover:shadow-lg transition-shadow cursor-pointer" onClick={() => setCurrentView('status')}>
            <CardHeader className="text-center pb-4">
              ...
              <CardTitle className="text-2xl text-gray-800">Am I Still Out?</CardTitle>
            </CardHeader>
            <CardContent className="text-center">
              <p className="text-gray-600 mb-6">Check your current bathroom pass status</p>
